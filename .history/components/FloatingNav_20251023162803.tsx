...
export const FloatingNav = () => {
  ...
  return (
    <motion.nav
      ...
      md:px-8
      ${isScrolled ? "bg-neutral-950" : "bg-neutral-950"}`}
    >
      <AnimatePresence>
        <motion.div
          key="nav-content"
          ...
          {!isOpen && !isSearchOpen && !isNavOpen && (
                ...
                <AnimatePresence mode="wait">
                  {!isScrolled ? (
                    ...
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -10 }}
                      transition={{ duration: 0.5 }}
                      className="flex items-center gap-4 text-amber-400 transition-colors duration-300"
                    >
                      <button
                        aria-label="Open menu"
                        onClick={() => setNavOpen(true)}
                      >
                        <Menu className="h-5 w-5 lg:h-6 lg:w-6 hover:cursor-pointer duration-300 ease-in-out hover:scale-105" />
                      </button>
                      ...
                      >
                        <Search className="w-5 h-5 lg:w-5 lg:h-5 hover:cursor-pointer duration-300 ease-in-out hover:scale-105" />
                      </button>
                    </motion.div>
                  )}
                </AnimatePresence>
            ...
            >
              <Link
                href="/"
                prefetch={true}
                className="text-amber-400 flex flex-col items-center h-fit uppercase"
                aria-label="Home"
              >

                <div className="ecurie">
                  <h2 className="text-[11px] sm:text-[12px] lg:text-sm capitalize">
                    Pristine de Luxe Model Kits
                  </h2>
                </div>
                <div className="-mt-2 font-extrabold">
                  <h2 className="text-[27px] sm:text-3xl lg:text-4xl logo tracking-wide">
                    Pocher
                  </h2>
                </div>
              ...
              <div className="relative group mt-[4px]">
                ...
                >
                  <ShoppingBag
                    size={26}
                    weight="duotone"
                    className="text-amber-400 transition-all duration-300 group-hover:scale-105"
                  />
                  {/*{itemCount > 0 && (
                    <span
