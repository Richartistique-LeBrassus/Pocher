...
export function FooterSection({ title, links }: FooterSectionProps) {
  ...
  return (
    <div className="border-b border-neutral-700 lg:border-none text-amber-400 dark:text-neutral-50">
      <button
        onClick={() => setOpen(!open)}
        className="w-full flex items-center justify-between py-7 lg:py-0 lg:cursor-default"
      >
        <h3 className="uppercase font-bold text-xs lg:text-sm tracking-widest
        lg:mb-4">{title}</h3>
        <ChevronDown
