      ...
      {visible &&
        createPortal(
          <div
            ...
            className={`fixed inset-0 z-[9999] h-[100vh] flex items-center justify-center bg-neutral-950 transition-opacity duration-300 ${
            ...
            <AnimatePresence>
              {open && (
                <motion.button
                  ...
                  className="absolute top-4 right-4 z-50 bg-white/30 p-2 text-black hover:bg-amber-400 rounded-xs hover:cursor-pointer transition"
...
>
  ...
  />
                ...
                <button
                  ...
                  className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/30 p-3 text-black hover:bg-amber-400 rounded-xs hover:cursor-pointer transition"
                >
                ...
                <button
                  ...
                  className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/30 p-3 text-black hover:bg-amber-400 rounded-xs hover:cursor-pointer transition"
                >
                  <ChevronRight className="h-6 w-6" />
                </button>
              </div>

              <div className="w-full max-w-[1100px] text-center text-sm text-neutral-100">
                <div className="text-xs relative h-4 flex items-center justify-center overflow-hidden">
                  <AnimatePresence mode="wait">
                    <motion.div
                      ...
                      className="absolute"
                    >
                      <span className="text-amber-400 underline underline-offset-4">{index + 1}</span> / {images.length}
                    </motion.div>
                  </AnimatePresence>
                </div>
              </div>
  ...
  <div className="w-full flex justify-center gap-3 px-2">
    {images.map((img, i) => (
      <motion.button
        ...
        aria-label={`Open image ${i + 1}`}
        className="relative h-20 min-w-[80px] shrink-0 overflow-hidden  focus:outline-none hover:cursor-pointer rounded-xl"
        whileHover={{ scale: 1.05 }}
        transition={{ type: "spring", stiffness: 300, damping: 20 }}
      >
        <Image
