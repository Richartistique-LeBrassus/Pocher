...
export function FooterSection({ title, links }: FooterSectionProps) {
  ...
  return (
    <div className="border-b border-neutral-700 lg:border-none text-amber-400 dark:text-neutral-50 tracking-widest">
      <button
        onClick={() => setOpen(!open)}
        className="w-full flex items-center justify-between py-7 lg:py-0 lg:cursor-default"
      >
        <h3 className="uppercase font-bold text-xs lg:text-sm
        lg:mb-4">{title}</h3>
        <ChevronDown
      ...
      <ul
        className={`overflow-hidden transition-[max-height] duration-300 lg:block space-y-1 text-neutral-100
          lg:mb-7 lg:w-4/5 ${
        ...
        {links.map(({ label, href }) => (
          <li key={label}>
            <Link
              ...
              transition-all duration-400 text-xs lg:text-sm
              font-medium"
            >
              {label}
