...
const SearchDrawer: React.FC<SearchDrawerProps> = ({ isOpen, onClose, children }) => {
  ...
  return (
    <>
      <div
        className={`fixed inset-0 bg-white/20 backdrop-blur-[120px]
      ...
      <motion.aside
        ...
        className="fixed top-0 right-0 h-full w-full
          lg:w-[50vw] text-center bg-neutral-950 z-[9999] shadow-xl py-36 overflow-y-auto
          md:rounded-tl-4xl pt-20 pb-5 px-9 md:pt-28 2xl:pt-36"
        role="dialog"
        aria-modal="true"
      >
        <button
          ...
          className="absolute top-4 right-4 text-neutral-100
        ...
        <Form action="/search">
          <input
            ...
            className="w-3/4 md:w-3/5 lg:w-3/4 2xl:w-3/5
              pb-2 border-b border-neutral-100 text-wide uppercase
              placeholder:text-neutral-100 placeholder:text-wide
              focus:outline-none transition text-amber-400"
          />
        </Form>
        ...
        {results.length > 0 && (
          <ul className="mt-6 space-y-3">
            {results.map((item) => (
              <li
                key={item.objectID}
                className="text-stone-50 border-b border-zinc-200 pb-2"
              >
                {item.name}
              </li>
            ))}
