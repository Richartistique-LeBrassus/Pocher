    ...
    >
      {videoList.map((video, index) => {
        ...
        return (
          <div
            key={index}
            className={`absolute w-full h-full transition-opacity duration-700 bg-neutral-950 ${
              index === currentVideo ? "opacity-100 z-10" : "opacity-0 z-0"
            }`}
          >
            {loading && index === currentVideo && (
              <div className="absolute inset-0 bg-neutral-950 rounded-4xl z-0" />
            )}
            ...
            {isVideo ? (
              <video
                ...
                className="w-full h-full object-cover rounded-4xl relative z-10"
